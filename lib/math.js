...
module.exports = {
  ...
  generateGaussianSample: (mean, standardDeviation) => {
    ...
    if (samplePool.length) return (mean + samplePool.pop() * standardDeviation).toFixed(2)
    ...
    samplePool.push(y * s)
    return (mean + x * s * standardDeviation).toFixed(2)
  },
  linearInterpolation: (min, max, interval) => {
    ...
    for (let i = 0; i <= interval; i++)
      values.push(Math.round(min + i * delta))
