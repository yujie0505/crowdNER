...
module.exports = {
  ...
  mean: data => data.reduce((sum, value) => sum + value, 0) / data.length,
  standardDeviation: (data, mean) => Math.sqrt(data.reduce((deviation, datum) => deviation + Math.pow((datum - mean), 2), 0) / data.length)
}
